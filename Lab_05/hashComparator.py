    ...
    if len(hash1) != len(hash2):
        ...
        for i in range(len(hash1)):
            ...
            if len(hash1[i]) != len(hash2[i]):
                ...
                compare(hashString1, hashString2, counter, hashes[counter], hashes[counter+1])
            counter += 1
def main():
    personal = prepareBin(hashpersonal)
    personal_ = prepareBin(hashpersonal_)
    compareHashes(personal, personal_, hashes)
